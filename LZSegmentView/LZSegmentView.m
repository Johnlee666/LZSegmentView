...
@interface LZSegmentView()<UIScrollViewDelegate>
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (currentIndex != _currentIndex) {
        ...
        [UIView animateWithDuration:0.2f animations:^{
            ...
            if (_btnArray[currentIndex].frame.origin.x < self.frame.size.width/2) {
                [_titleScrollView setContentOffset:CGPointMake(0, 0) animated:YES];
                //在屏幕中线左边的情况
            } else if (_titleScrollView.contentSize.width - _btnArray[currentIndex].frame.origin.x <= self.frame.size.width/2) {
                [_titleScrollView setContentOffset:CGPointMake(_controllers.count*_titleBtnWidth-self.frame.size.width, 0) animated:YES];
                //在最右边的情况
            } else {
                [_titleScrollView setContentOffset:CGPointMake(_btnArray[currentIndex].frame.origin.x-self.frame.size.width/2+_titleBtnWidth/2, 0) animated:YES];
                //在中间的情况
            }
        } completion:nil];
